...
void CDlgMPLFName::OnBnClickedFNumLoc()
{
	...
	rt = gcedGetCorner(asDblArray(pnt1), _T("도면번호 문자에 해당하는 두번째 모서리 점 지정:"), asDblArray(pnt4));
	if (rt != RTNORM)
		goto ErrorHandler;
...
void CDlgMPLFName::OnBnClickedFNamLoc()
{
	...
	rt = gcedGetCorner(asDblArray(pnt1), _T("도면이름 문자에 해당하는 두번째 모서리 점 지정:"), asDblArray(pnt4));
	if (rt != RTNORM)
		goto ErrorHandler;

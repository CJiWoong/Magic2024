...
LPTSTR GetFormat(int iDec)
{
	...
	if (iDec < 0)
		iDec = 0;
	_stprintf_s(fmtStr, 255, _T("%%.%df"), iDec);
...
int GetUcsPoint(gds_point bgnPnt, LPCTSTR prompt, gds_point inpPnt)
{
	...
	if (rt == RTNORM)
		gcdbWcs2Ucs(wcsPnt, inpPnt, Gsoft::kTrue);
...
BOOL IsWCS()
{
	...
	gcdbUcs2Wcs(asDblArray(pTemp), asDblArray(vXVect), Gsoft::kTrue);
	GetVariable(_T("UCSYDIR"), pTemp);
	//vYVect = .Utility.TranslateCoordinates(pTemp, acUCS, acWorld, 0);
	gcdbUcs2Wcs(asDblArray(pTemp), asDblArray(vYVect), Gsoft::kTrue);
...
LPTSTR GetLayerState(LPCTSTR layNam, BOOL resMsg)
{
	...
	if (layPtr->isFrozen()) {
		_stprintf_s(retPtr, 255, _T("%s 레이어 : 동결\n"), pLName); retPtr += _tcslen(retPtr);
	}
	if (layPtr->isLocked()) {
		_stprintf_s(retPtr, 255, _T("%s 레이어 : 잠김\n"), pLName); retPtr += _tcslen(retPtr);
	}
	if (layPtr->isOff()) {
		_stprintf_s(retPtr, 255, _T("%s 레이어 : 꺼짐\n"), pLName); retPtr += _tcslen(retPtr);
	}
	layPtr->close();

	if (retMsg[0] != '\0' && resMsg) {
		_tcscpy_s(retPtr, 255, _T("기본 레이어로 설정")); retPtr += _tcslen(retPtr);
	}
...
LPTSTR LockBackUp()
{
	...
	for (; !layItr->done(); layItr->step()) {
		...
		if (layPtr->isLocked()) {
			...
			_tcscpy_s(strPtr, 1023, layNam); strPtr += _tcslen(strPtr);
			_tcscpy_s(strPtr, 1023, _T(",")); strPtr += _tcslen(strPtr);
...
LPCTSTR GetColorName(int iDefault, BOOL bMeta)
{
	...
	switch(iColor) {
	case 256:
		_tcscpy_s(colNam, 255, _T("By Layer"));
		break;
	case 0:
		_tcscpy_s(colNam, 255, _T("By Block"));
		break;
	default:
		_stprintf_s(colNam, 255, _T("%d"), iColor);
	}
...
void CreateSelSet()
{
	...
	if (res == RTNORM) {
		...
		// test code
		gcedCommandS (RTSTR, "._SELECT", RTPICKS, ss, RTSTR, "", RTNONE);
...
{
	INT_PTR iArraySize = csa.GetSize();
	if (iArraySize <= 0)
		return;

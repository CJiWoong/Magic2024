...
static AFX_EXTENSION_MODULE MagicUtilDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("MagicUtil.DLL을 초기화하고 있습니다.\n");
		
		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(MagicUtilDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MagicUtilDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("MagicUtil.DLL을 종료하고 있습니다.\n");

		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(MagicUtilDLL);
	}
	return 1;   // 확인
}

...
void SetupMagic()
{
	...
	//HKEY_CURRENT_USER\SOFTWARE\Gstarsoft\GstarMechStd\R21\ko-KR\Profiles\GstarMech2022Pro\Config\STARTUP
	keyPath = _T("Software\\Gstarsoft\\GstarMechStd\\R22\\ko-KR\\Profiles");
	regEdit.SetKey(HKEY_CURRENT_USER, keyPath);
	CString proNam = regEdit.GetString(_T(""), _T(""));

	// get number of Startups
	keyPath += _T("\\") + proNam + _T("\\Dialogs\\Appload\\Startup");
//	keyPath += _T("\\Dialogs\\Appload\\Startup");
//	keyPath += _T("\\") + proNam + _T("\\Config\\Startup");
	regEdit.SetKey(HKEY_CURRENT_USER, keyPath);

...
void SetupMagic()
{
	...
	//이건 한국어 버전
	//HKEY_CURRENT_USER\SOFTWARE\Gstarsoft\GstarCAD\R23\ko-KR\Profiles\<<명명되지 않은 프로파일>>\Config\STARTUP
	//아래는 영문 버전
	//HKEY_CURRENT_USER\SOFTWARE\Gstarsoft\GstarCAD\R23\en-WW\Profiles\<<Unnamed Profile>>\Config\STARTUP
	keyPath = _T("Software\\Gstarsoft\\GstarCAD\\R23\\ko-KR\\Profiles\\<<명명되지 않은 프로파일>>");
	regEdit.SetKey(HKEY_CURRENT_USER, keyPath);
